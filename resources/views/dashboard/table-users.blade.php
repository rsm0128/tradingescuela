    <select id="payment" style="margin-bottom:30px">
        ...
        <option value="{{ route('manage-user') }}">Todos</option>
        <option value="{{ route('manage-user-paid') }}">Pagado</option>
        <option value="{{ route('manage-user-not-paid') }}">Sin Pagar</option>
    </select>
        ...
        <table class="table table-striped table-bordered table-hover" id="sample_1">
            ...
            @foreach($user as $k => $p)
                <tr class="{{ ($p->payment_type == NULL) ? 'bg-warning ' : (($p->payment_type != NULL && $p->plan_status == 0) ? 'bg-danger' : '') }}">
                    <td>{{ $p->id }}</td>
                    <td><a href="{{ route('user-edit',$p->id) }}" class="btn btn-primary btn-sm bold uppercase" title="Edit"><i class="fa fa-edit"></i> Editar</a></td>
